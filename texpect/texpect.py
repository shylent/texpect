...
class Promise(Deferred):
    r"""Base class for Deferred extensions, that are used in TExpect.
...
class TExpect(telnet.Telnet):
    ...
    def __init__(self, debug=False, timeout=None, *args, **kwargs):
        r"""
        ...
        """
        telnet.Telnet.__init__(self, *args, **kwargs)
    ...
    def expect(self, pattern_list, timeout=None, _promise_class=Expect):
        ...
        if res:
            ...
            if timeout is not None:
                ...
                promise._timeout = reactor.callLater(timeout, self._handle_timeout)
        return promise
